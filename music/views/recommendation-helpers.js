// Shared recommendation-shelf computation used by both the full Discover
// page (views/discover.js) and the Home page's teaser shelf (views/home.js)
// -- so the homepage genuinely surfaces the same deep-cuts/completion-based
// picks Discover does, rather than a separate "recently popular" query
// (which was redundant with Home's own precomputed "Top Releases This
// Month" section). Memoized module-level: the underlying listens/tracks
// data never changes mid-session, so there's no reason to re-run these ~13
// SQL queries on every Home/Discover mount.

let _recShelvesCache = null;

function computeRecommendationShelves(db) {
    if (_recShelvesCache) return _recShelvesCache;

    const seed = db.exec('SELECT COUNT(*) FROM listens')[0].values[0][0];
    ...
    function favoritesThisYear(now) {
    ...
    function favoritesPerYear(currentYear, exclude) {
        ...
        years.forEach((year, idx) => {
            ...
            const pool = q(`
                ...
                GROUP BY r.id ORDER BY plays DESC LIMIT 30
            `);
            const chosen = pick(pool, 1, idx + 100, seen);
            if (chosen.length) {
                picks.push(chosen[0]);
                seen.add(chosen[0][0]);
            }
    ...
    function throwbacks(currentYear) {
        return q(`
            SELECT ${COLS}, COUNT(l.id) plays
            FROM listens l
            JOIN tracks t ON l.track_id = t.id
    ...
    function thisMonthPastYears(currentMonth, currentYear) {
        return q(`
            SELECT ${COLS}, COUNT(l.id) plays
            FROM listens l
            JOIN tracks t ON l.track_id = t.id
    ...
    function moreFromThisArtist(now) {
    ...
    function anniversary(currentMonth, now) {
        const cutoff = now - 365 * 86400;
        return q(`
            SELECT ${COLS},
    ...
    function soundtrackSpotlight(now) {
    ...
    // Global seen set -- shelves later in the list won't repeat albums shown
    // earlier. Order matters: higher-priority shelves claim first.
    const seen = new Set();

    function picked(pool, n, idx) {
        const rows = pick(pool, n, idx, seen);
        rows.forEach(r => seen.add(r[0]));
        return rows;
    }

    const favYear   = picked(favoritesThisYear(now), SHELF_DESKTOP, 0);

    // favoritesPerYear manages its own internal dedup + respects global seen
    const perYear   = favoritesPerYear(cy, seen);
    perYear.forEach(r => seen.add(r[0]));

    const throwback  = picked(throwbacks(cy), SHELF_DESKTOP, 2);
    const thisMonth  = picked(thisMonthPastYears(cm, cy), SHELF_DESKTOP, 3);
    const risingCards = picked(rising(now), SHELF_DESKTOP, 4);
    const fading     = picked(fadingFavorites(now), SHELF_DESKTOP, 5);
    const oneAway    = picked(oneTrackAway(), SHELF_DESKTOP, 6);
    const deepCut    = picked(deepCutNeeded(), SHELF_DESKTOP, 7);
    const heardOnce  = picked(onlyHeardOnce(now), SHELF_DESKTOP, 8);

    const moreResult = moreFromThisArtist(now);
    ...
    const anniv      = picked(anniversary(cm, now), SHELF_DESKTOP, 10);
    const soundtrack = picked(soundtrackSpotlight(now), SHELF_DESKTOP, 11);
    const shortForm  = picked(shortFormFavourites(), SHELF_DESKTOP, 12);
    ...
    const shelves = [
        ...
        ['Rising',                     'Albums with more plays in the last 30 days than the 30 before.',           risingCards],
    ...
    _recShelvesCache = shelves.map(([title, desc, rows]) => [title, desc, cardsFrom(rows)]);
    return _recShelvesCache;
}

// Shelf titles to sample from for Home's small teaser, in priority order --
// weighted toward genuine deep-cut/completion categories (a reason to click
// through) over recently-popular ones. 'Favorites This Year' and 'Rising'
// are deliberately excluded: both are "top albums by recent play count"
// under the hood, which is redundant with Home's own separately-computed
// "Top Releases This Month" section directly above this one.
const TEASER_SHELF_ORDER = [
    'One Track Away', 'Deep Cut Needed', 'Only Heard Once', 'Fading Favorites',
    'Anniversary', 'Throwbacks', 'Soundtrack Spotlight', 'Short-Form Favourites',
    'More From This Artist', 'This Month, Past Years', 'Favorites From Last 5 Years',
];

// Round-robins one card per shelf (in TEASER_SHELF_ORDER, skipping empty
// ones) until `n` are picked, wrapping to each shelf's 2nd/3rd card if there
// aren't enough shelves -- gives Home's small teaser grid category variety
// instead of dumping n cards from a single shelf.
function pickTeaserCards(shelves, n) {
    const byTitle = new Map(shelves.map(([title, , cards]) => [title, cards]));
    const ordered = TEASER_SHELF_ORDER
        .map(title => byTitle.get(title))
        .filter(cards => cards && cards.length);
    if (!ordered.length) return [];
    const picked = [];
    for (let round = 0; picked.length < n && round < 5; round++) {
        for (const cards of ordered) {
            if (picked.length >= n) break;
            if (cards[round]) picked.push(cards[round]);
        }
    }
    return picked;
}

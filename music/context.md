...
├── index.html                   # Homepage with top artists/albums
├── year.html                    # Year-specific view (2011-2025)
├── artist.html                  # Artist detail page
├── release.html                 # Release/album detail page
├── config.js                    # Database configuration (version, URL)
├── app.js                       # Homepage logic (7 KB)
├── year.js                      # Year page logic (10 KB)
├── artist.js                    # Artist page logic (11 KB)
...
└── listening_history.sqlite     # SQLite database (56 MB)
```
...
3. **Top Artists & Top Albums** (side-by-side on desktop, sortable, multiple views)
   - Layout: Two-column grid on desktop (≥769px), single column on mobile
   - 🎧 Sort by play count (default)
   - ⏱️ Sort by listening time
   - 📅 Sort by release date (albums only, newest first)

   **View System** (2 independent toggles):
   - **Mode Toggle (📋 List button)**: Grid vs List layout
     - Grid: Square artwork cards
     - List: Text-only rows
   - **Stats Toggle (👁️ Eye button)**: Show/hide statistics
   ...
   **Controls Layout:**
   ...
   ```
   - **👁️ (Eye)**: Toggles stats on/off (works in both grid and list modes)
   - **📋 (List)**: Switches between grid and list layout
...
**Mobile**: Stacks vertically (photo → stats → chart → releases → tracks)
...
Summary: N artists · M albums · X plays
...
3. **Top Artists** (same view system as homepage)
   - Layout: Side-by-side with albums on desktop (≥769px)
   - 🎧 Sort by play count (default)
   - ⏱️ Sort by listening time
   - **View System** (2 independent toggles):
     - **👁️ Eye button**: Toggle stats overlay
     - **📋 List button**: Toggle grid/list layout
   - Shows artists listened to during that year
   - Displays headphone (listens) and clock (minutes) icons
   - All 4 view combinations available (grid/list × stats on/off)

4. **Top Albums** (same view system + filter)
   - 🎧 Sort by play count (default)
   - ⏱️ Sort by listening time
   - **🔍 Filter button**: Toggle album filtering mode
...
**Mobile**: Stacks vertically (year nav → summary → artists → albums)
...
- **Listens mode**: "1,234 plays · 567 min"
- **Minutes mode**: "567 min · 1,234 plays"
- **Date mode**: "2024 · 1,234 plays"
...
@media (max-width: 768px) {
    ...
    /* Two-section-layout: 2 columns → 1 column */
}
```
...
```
Homepage
  ├─> Search → Artist Page → Release Page
  ├─> Top Artists → Artist Page
  ├─> Top Albums → Release Page
  └─> Year Link (2011-2025) → Year Page
...
- Desktop (≥769px): Side-by-side two-column layout for Artists/Albums
- Mobile (<768px): Stacked single-column layout

**View Toggle Controls:**
- Two independent icon-based buttons (👁️ eye, 📋 list)
...
**Changes on mobile:**
- Logo scales down (60px → 50px)
- Stats grid: 4 columns → 2 columns
- **Two-section-layout: side-by-side → stacked (3rem gap)**
- Artist header: horizontal → vertical stack
- Artist photo: 150px → 120px
- Chart controls: horizontal → vertical stack, full width
- Two-column sections → single column
- Sort buttons: expand to fill width
- Image grid: tighter spacing (1rem → 0.75rem, 180px → 140px min)
...
After deduplication:
- Artist B: "Kendrick Lamar" (mbid-xyz-123, with image)
  → All track_artists and listens now reference mbid-xyz-123
```
...
### Why SQLite WASM?
- ✅ 56 MB is acceptable for modern browsers
...
**Database Loading:**
- If `music/listening_history.sqlite` exists locally → uses local file (faster for testing)
- If local file missing → automatically falls back to GitHub Releases CDN
...
- 📅 **14 years** of listening (2011-2025)
- 🎧 **124,483 listens**
- 🎵 **8,480 artists**
- 💿 **14,556 releases**
- 🎼 **42,153 unique tracks**
- 🤝 **2,900 featured** + **488 collaboration** credits
...
- ✅ Multiple view modes with independent toggles (4 combinations)
- ✅ Clear active states on all toggle buttons
- ✅ Side-by-side desktop layout
- ✅ Name-only list view implemented
- ✅ Spotify integration for high-quality images (640x640 square)
...
- Accessible from homepage via "2011 – 2025" link
...
- Same sorting system as homepage (🎧 listens, ⏱️ minutes)
- Same view system as homepage (👁️ stats toggle, 📋 list/grid toggle)
- Shows headphone and clock icons for all entries
- Side-by-side layout on desktop, stacked on mobile
- All 4 view combinations available (grid/list × stats on/off)

**Album Filter Feature (Unique to Year Page):**
- 🔍 Filter button toggles between two modes:
...
- Positioned in controls group: `[🎧] [⏱️] | [🔍] [👁️] [📋]`
...
**Renamed script:**
- `lastfm_parser.py` → `lastfm_importer.py`
...
- Eye button (👁️) and List button (📋) now work independently
